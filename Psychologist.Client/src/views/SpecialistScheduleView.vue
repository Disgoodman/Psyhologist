...
async function clickDay(event) {
  ...
  if (event.events.length !== 0) {
    await router.push({ name: 'scheduleDay', params: { specialistId: store.state.auth.user.specialist.id, date: startDate.toISODate() } })
  }
}
...
onMounted(async () => {
  ...
  let data = await callGet(`/api/schedule`);

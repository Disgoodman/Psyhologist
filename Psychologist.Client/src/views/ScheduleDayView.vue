    ...
    <div class="container-md mt-3">
      <div class="list-group">
        <router-link v-for="interval in schedule?.consultations"
                     ...
                     :to="{ name: 'consultation', params: { date: props.date.toISODate(), time: interval.start.toFormat('HH:mm') } }"
          ...
          <template v-if="interval.consultation">
            ...
            <template v-if="interval.consultation.type === 'individualConsultation'">
              ...
              <p class="mb-1">Характер консультации: {{ interval.consultation.nature }}</p>
            </template>
...
onMounted(async () => {
  let rawSchedule = await callGet('/api/schedule/' + props.date.toISODate());
